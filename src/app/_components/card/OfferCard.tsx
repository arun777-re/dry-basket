...
e.preventDefault();
dispatch(deleteoffer(_id)).unwrap().then((res)=>{
  ...
  return (
    <Card size={"4"} className="w-[36vw] h-[240px] shadow-xl relative rounded-md">
      <div className="p-4 w-full h-full relative flex flex-col items-center gap-8">
        <article className="w-full h-auto relative flex flex-wrap gap-4">
          ...
          <p>expiresAt:{expiresAt ? new Date(expiresAt).toLocaleDateString() : "N/A"}</p>
          <p>used:{timesUsed}</p>
          <p>limit:{usageLimit}</p>
        </article>
        <div className="w-full h-auto flex gap-4 justify-between">
            <Button type="button" onClick={DeleteOffer} className="px-3 py-2 border-2 border-first bg-white hover:bg-first transition-all duration-500 ease-in-out cursor-pointer">
                Delete Offer
            </Button>
            <Button type="button" onClick={()=>setPop(true)} className="px-3 py-2 border-2 border-first bg-white hover:bg-first transition-all duration-500 ease-in-out cursor-pointer">
                Update Offer
            </Button>
        </div>
      </div>
      ...
      {pop === true && <UpdateForm _id={_id}/>}
       <MdCancel onClick={() => setPop((prev) => !prev)} size={30} className="absolute top-0 right-0 cursor-pointer bg-white"/>
      </div>
    </Card>
  );
};
