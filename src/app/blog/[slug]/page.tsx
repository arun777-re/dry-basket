...
import { FaFacebook, FaGoogle, FaInstagram, FaRegUserCircle, FaTwitter } from "react-icons/fa";
...
<h4 className='text-body '>Black pepper powder</h4>
<div className="flex flex-row items-center justify-center pt-1 gap-2 ">
    {[
        {icon:<SlCalender size={14} className='text-head'/>,text:"Nov 13, 2018"},
        {icon:<LiaComments size={18} className='text-head'/>,text:"6 Comments"},
        {icon:<FaRegUserCircle size={16} className='text-head'/>,text:"Ram M"},
...
<p className="text-black text-base font-medium font-cursive py-3">
   "Lorem ipsum dolor sit amet consectetur, adipisicing elit. Veniam, mollitia. Porro fugiat doloribus optio quasi ipsum eos ducimus a tempore!"
</p>
<p className='leading-relaxed'>
    Lorem ipsum dolor, sit amet consectetur adipisicing elit. Eveniet voluptatem aperiam nulla tempora omnis culpa perspiciatis, natus, architecto magnam libero, neque voluptatum quaerat est accusantium reprehenderit vel dolores nam
     temporibus blanditiis aliquam. Accusamus dolorum doloribus facilis nisi cupiditate soluta quaerat.
</p>
...
<div className="w-full h-auto relative pt-6">
    <div className="px-8 py-6 w-full relative flex flex-col items-start justify-center gap-8 bg-gray-100">
        ...
        {[
            {createdAt:"March 6,2025",createdBy:"Bharat",comment:"Best product and best services are found to this store"},
            {createdAt:"March 10,2025",createdBy:"Arun",comment:"Best product and best services are found to this store"},
            {createdAt:"March 20,2025",createdBy:"Katiya",comment:"Best product and best services are found to this store"},
        ].map((value,key)=>{
            return (
<div className="relative w-full flex items-center gap-2">
    ...
    <article className="w-full h-auto relative flex flex-col items-start px-5 py-6 bg-white">
        <div className="flex relative items-center w-full gap-2">
           ...
           {[
        {icon:<SlCalender size={14} className='text-head'/>,text:value?.createdAt},
        {icon:<FaRegUserCircle size={16} className='text-head'/>,text:value?.createdBy},

].map((item,index)=>{
    return (
        <article className="flex flex-row gap-1 items-center" key={index}>
            {item?.icon}
            <p>{item?.text}</p>
        </article>
    )
...
<p>{value?.comment}</p>
        ...
        <div className="flex flex-col items-start gap-8 relative w-full">
            ...
            <form action="" method="post" className='w-full relative grid  gap-6'>
                <div className="grid grid-cols-2 gap-6 relative w-full">
                <input type="text" placeholder='Name'  className='w-full px-4 py-3  outline-1
                ...
                <input type="text" placeholder='Email'  className='w-full px-4 py-3 outline-1
                transition-all duration-300 ease-in-out rounded-full focus:outline-head bg-white placeholder:text-body placeholder:text-start placeholder:text-sm'/>
                </div>
                <input type="text" 
  ...
  <Button type='submit' className=' border-2 border-head hover:border-first hover:bg-first transition-all duration-500 ease-in-out'>
                    Post comment
                </Button>
                </div>
